...
const AsistenciaAdmin = () => {
  ...
  return (
    <div class="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b-2 border-carbon dark:border-white/20 pb-4">
        ...
        {activeTab === 'tomar' && (
          <div class="flex items-center gap-3">
            <div class="flex items-center gap-2 bg-white dark:bg-[#0A0B0E] border border-carbon/20 dark:border-white/10 px-3 py-1.5 rounded-lg shadow-inner">
              <Calendar size={16} class="text-carbon dark:text-white" />
              <input
                type="date"
                value={fecha}
                onChange={(e) => setFecha(e.target.value)}
                class="bg-transparent text-carbon dark:text-white text-sm focus:outline-none cursor-pointer border-none font-bold"
              />
            </div>
      ...
      <div class="flex overflow-x-auto whitespace-nowrap border-b border-carbon/20 dark:border-white/10 gap-2 pb-1" style={{ scrollbarWidth: 'none' }}>
        <button
          ...
          class={`flex-shrink-0 px-4 sm:px-6 py-3 text-[10px] sm:text-xs font-extrabold uppercase tracking-widest transition-all border-b-2 flex items-center gap-2 ${
            activeTab === 'tomar'
              ...
              : 'border-transparent text-gray-600 dark:text-gray-400 hover:text-carbon dark:text-white'
          }`}
        >
      ...
      {activeTab !== 'historial' && (
        <div class="bg-white dark:bg-[#0A0B0E]/90 border border-carbon/20 dark:border-white/10 p-4 rounded-xl grid grid-cols-1 md:grid-cols-3 gap-4 shadow-xl">
          {/* Buscar */}
          <div class="relative flex items-center bg-white dark:bg-[#0A0B0E] rounded-lg border border-carbon/20 dark:border-white/10 px-3 py-2">
            <Search size={16} class="text-gray-600 dark:text-gray-400 mr-2" />
            <input
              type="text"
              placeholder="Buscar por nombre, apellido o cédula..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              class="bg-transparent text-xs text-carbon dark:text-white placeholder-gray-500 w-full focus:outline-none"
            />
          </div>
          ...
          <div class="relative md:col-span-2">
            <button
              ...
              class="flex items-center gap-2 bg-white dark:bg-[#0A0B0E] rounded-lg border border-carbon/20 dark:border-white/10 px-4 py-2 text-xs text-carbon dark:text-white hover:bg-black/5 dark:hover:bg-white/5 transition-colors w-full md:w-auto"
            >
              <Filter size={16} class="text-carbon dark:text-white" />
              <span class="font-bold uppercase tracking-wider">Filtros</span>
              {(selectedModalidad) && (
                <span class="ml-2 w-2 h-2 rounded-full bg-carbon dark:bg-white"></span>
              )}
            </button>

            {filtersOpen && (
              <div class="absolute top-full left-0 mt-2 w-full md:w-80 bg-white dark:bg-[#0A0B0E] border border-carbon/20 dark:border-white/10 rounded-xl shadow-2xl p-4 z-50 space-y-4">
                {/* Disciplina Dropdown */}
                <div class="space-y-1">
                  <label class="text-[10px] text-gray-600 dark:text-gray-400 font-bold uppercase tracking-widest">Disciplina</label>
                  <select
                    value={selectedModalidad}
                    onChange={(e) => setSelectedModalidad(e.target.value)}
                    class="bg-gray-50 dark:bg-[#1C1C21] text-xs text-carbon dark:text-white border border-carbon/20 dark:border-white/10 rounded-lg p-2 w-full focus:outline-none focus:border-carbon dark:border-white/20"
                  >
                    <option value="" class="bg-gray-50 dark:bg-[#1C1C21]">Todas las Disciplinas</option>
                    <option value="TAEKWONDO" class="bg-gray-50 dark:bg-[#1C1C21]">Taekwondo</option>
                    <option value="KICKBOXING" class="bg-gray-50 dark:bg-[#1C1C21]">Kickboxing</option>
                  </select>
                </div>
              </div>
            )}
      ...
      {activeTab === 'tomar' && (
        <div class="space-y-6">
          ...
          <div class="flex items-center justify-between border-b border-carbon/10 dark:border-white/5 pb-2">
            <h3 class="text-xs font-bold text-gray-600 dark:text-gray-400 font-display tracking-widest uppercase font-body">Resumen del Día</h3>
            <button
              onClick={() => setShowStats(!showStats)}
              class="text-[10px] text-carbon dark:text-white hover:text-carbon dark:text-white transition-colors flex items-center gap-1.5 font-bold uppercase tracking-widest bg-white dark:bg-[#0A0B0E] border border-carbon/30 dark:border-white/20 px-3 py-1.5 rounded-lg"
            >
              {showStats ? (
          ...
          {showStats && (
            ...
            <div class="bg-emerald-950/20 border border-emerald-500/20 p-4 rounded-xl text-center space-y-1">
              ...
              <div class="text-2xl font-bold text-emerald-400 font-body">{stats.presentes}</div>
            ...
            <div class="bg-rose-950/20 border border-rose-500/20 p-4 rounded-xl text-center space-y-1">
              ...
              <div class="text-2xl font-bold text-rose-400 font-body">{stats.ausentes}</div>
            ...
            <div class="bg-amber-950/20 border border-amber-500/20 p-4 rounded-xl text-center space-y-1">
              ...
              <div class="text-2xl font-bold text-amber-400 font-body">{stats.tardes}</div>
            ...
            <div class="bg-blue-950/20 border border-blue-500/20 p-4 rounded-xl text-center space-y-1">
              ...
              <div class="text-2xl font-bold text-blue-400 font-body">{stats.justificados}</div>
              <span class="text-[9px] text-blue-500/80">Faltas notificadas</span>
            </div>

            <div class="bg-white dark:bg-[#0A0B0E]/50 border border-carbon/10 dark:border-white/5 p-4 rounded-xl text-center space-y-1 col-span-2 md:col-span-1">
              <span class="text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest block">Sin Registrar</span>
              <div class="text-2xl font-bold text-carbon dark:text-white font-body">{stats.sinRegistrar}</div>
          ...
          <div class="bg-white dark:bg-[#0A0B0E]/70 border border-carbon/20 dark:border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            {loading ? (
              <div class="flex flex-col items-center justify-center py-20 gap-4">
                <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-carbon dark:border-white/20"></div>
                <p class="text-xs text-gray-600 dark:text-gray-400">Cargando fichas de alumnos...</p>
              </div>
            ) : students.length === 0 ? (
              <div class="text-center py-16 px-4 space-y-3">
                <Users size={48} class="text-gray-600 dark:text-gray-400 mx-auto" />
                <h3 class="text-base font-body font-semibold text-carbon dark:text-white uppercase">No se encontraron estudiantes</h3>
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                  <thead>
                    <tr class="bg-white dark:bg-[#0A0B0E] border-b border-carbon/20 dark:border-white/10 text-[10px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-widest">
                  ...
                  <tbody class="divide-y divide-carbon/10 dark:divide-white/5 text-xs">
                    {students.map((student) => {
                      const currentEstado = student.attendance?.estado;
                      const isUpdating = updatingId === student.id;

                      return (
                        <tr key={student.id} class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                          <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                              {student.foto ? (
                                <img
                              ...
                              <div>
                                <span class="font-bold text-carbon dark:text-white block">{student.nombreCompleto}</span>
                                <span class="text-[10px] text-gray-600 dark:text-gray-400 block sm:hidden">C.I: {student.cedula}</span>
                              </div>
                            </div>
                          </td>

                          <td class="py-4 px-6 text-gray-700 dark:text-gray-300 font-mono hidden sm:table-cell">
                            {student.cedula}
                          </td>

                          <td class="py-4 px-6 text-gray-600 dark:text-gray-400 hidden md:table-cell">
                            {student.club?.nombre || 'Sin Club asignado'}
                          </td>

                          <td class="py-4 px-6">
                            <div class="space-y-0.5">
                              <span class={`inline-block px-2 py-0.5 rounded text-[9px] font-bold tracking-wider border ${
                                student.modalidad === 'TAEKWONDO'
                                  ? 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-500/20'
                                  : 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-500/20'
                              }`}>
                                {student.modalidad}
                              </span>
                              <span class="text-[10px] text-gray-600 dark:text-gray-400 block">{student.grado}</span>
                            </div>
                          </td>
                          ...
                          <td class="py-4 px-6">
                            <div class="flex items-center justify-center gap-2">
                              ...
                              <button
                                ...
                                class={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold tracking-widest transition-all flex items-center gap-1 ${
                                  currentEstado === 'PRESENTE'
                                    ? 'bg-emerald-600 border-emerald-500 text-carbon dark:text-white shadow-lg shadow-emerald-950/40'
                                    : 'bg-white dark:bg-[#0A0B0E]/40 border-carbon/20 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-emerald-500/50 hover:text-emerald-400'
                                }`}
                              >
                                <CheckCircle size={12} />
                              ...
                              <button
                                ...
                                class={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold tracking-widest transition-all flex items-center gap-1 ${
                                  currentEstado === 'AUSENTE'
                                    ? 'bg-rose-600 border-rose-500 text-carbon dark:text-white shadow-lg shadow-rose-950/40'
                                    : 'bg-white dark:bg-[#0A0B0E]/40 border-carbon/20 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-rose-500/50 hover:text-rose-400'
                                }`}
                              >
                                <XCircle size={12} />
                              ...
                              <button
                                ...
                                class={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold tracking-widest transition-all flex items-center gap-1 ${
                                  currentEstado === 'TARDE'
                                    ? 'bg-amber-600 border-amber-500 text-carbon dark:text-white shadow-lg shadow-amber-950/40'
                                    : 'bg-white dark:bg-[#0A0B0E]/40 border-carbon/20 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-amber-500/50 hover:text-amber-400'
                                }`}
                              >
                                <Clock size={12} />
                              ...
                              <button
                                ...
                                class={`px-2.5 py-1.5 rounded-lg border text-[10px] font-bold tracking-widest transition-all flex items-center gap-1 ${
                                  currentEstado === 'JUSTIFICADO'
                                    ? 'bg-blue-600 border-blue-500 text-carbon dark:text-white shadow-lg shadow-blue-950/40'
                                    : 'bg-white dark:bg-[#0A0B0E]/40 border-carbon/20 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-blue-500/50 hover:text-blue-400'
                                }`}
                              >
                                <HelpCircle size={12} />
                              ...
                              {currentEstado && (
                                <button
                                  ...
                                  disabled={isUpdating}
                                  class="p-1.5 rounded-lg bg-white dark:bg-[#0A0B0E]/5 border border-carbon/20 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:bg-rojo-impacto hover:text-white hover:border-rojo-impacto transition-all"
                                >
                                  <RotateCcw size={12} />
                                </button>
                              )}
        ...
        <div class="space-y-6">
          <div class="bg-white dark:bg-[#0A0B0E]/70 border border-carbon/20 dark:border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            {loadingHistory ? (
              <div class="flex flex-col items-center justify-center py-20 gap-4">
                <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-carbon dark:border-white/20"></div>
                <p class="text-xs text-gray-600 dark:text-gray-400">Cargando fechas de asistencia históricas...</p>
              </div>
            ) : historyList.length === 0 ? (
              <div class="text-center py-16 px-4 space-y-3">
                <Calendar size={48} class="text-gray-600 dark:text-gray-400 mx-auto" />
                <h3 class="text-base font-body font-semibold text-carbon dark:text-white uppercase">No se encontraron registros de asistencia</h3>
                <p class="text-xs text-gray-600 dark:text-gray-400 max-w-md mx-auto">
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-carbon/10 dark:divide-white/5 text-xs text-center">
                    {historyList.map((h, i) => (
                      <tr key={i} class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                        {/* Fecha */}
                        <td class="py-4 px-6 text-left font-bold text-carbon dark:text-white">
                          <span class="inline-flex items-center gap-2">
                            <Calendar size={14} class="text-carbon dark:text-white" />
        ...
        <div class="space-y-6">
          <div class="bg-white dark:bg-[#0A0B0E]/70 border border-carbon/20 dark:border-white/10 rounded-2xl overflow-hidden shadow-2xl">
            {loadingReport ? (
              <div class="flex flex-col items-center justify-center py-20 gap-4">
                <div class="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-carbon dark:border-white/20"></div>
                <p class="text-xs text-gray-600 dark:text-gray-400">Calculando estadísticas de asistencia de alumnos...</p>
              </div>
            ) : reportList.length === 0 ? (
              <div class="text-center py-16 px-4 space-y-3">
                <Users size={48} class="text-gray-600 dark:text-gray-400 mx-auto" />
                <h3 class="text-base font-body font-semibold text-carbon dark:text-white uppercase">No hay alumnos para reportar</h3>
                <p class="text-xs text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                        ...
                        <tr key={student.id} class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                          ...
                          <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                              ...
                              <div>
                                <span class="font-bold text-carbon dark:text-white block">{student.nombreCompleto}</span>
                                <span class="text-[9px] text-gray-600 dark:text-gray-400">{student.grado}</span>
                              </div>
                            </div>
                          </td>

                          {/* Club */}
                          <td class="py-4 px-6 text-gray-600 dark:text-gray-400 hidden sm:table-cell">
                            {student.club}
                          </td>
                          ...
                          <td class="py-4 px-6">
                            <div class="flex items-center gap-3 max-w-xs">
                              <span class={`font-extrabold text-sm w-12 text-right font-body ${percentColor}`}>
                                {student.porcentaje}%
                              </span>
                              <div class="w-full bg-white dark:bg-[#0A0B0E]/10 h-2 rounded-full overflow-hidden">
                                <div
