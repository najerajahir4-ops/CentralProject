      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 class="text-2xl font-black text-carbon dark:text-white uppercase tracking-tight flex items-center gap-2 font-body">
            <User class="text-carbon dark:text-white" size={28} />
            Perfiles de Estudiantes
          </h2>
          <p class="text-gray-600 dark:text-gray-400 text-sm">Selecciona un estudiante para ver su perfil completo y su galería de progreso.</p>
        </div>
      </div>

      <div class="bg-gray-50 dark:bg-[#1C1C21] border border-carbon/20 dark:border-white/10 p-4 rounded-sm">
        <div class="relative max-w-md">
          <Search class="w-4 h-4 text-carbon dark:text-white absolute left-3 top-3" />
          <input
            type="text"
            placeholder="Buscar por nombre o cédula..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            class="w-full bg-gray-50 dark:bg-[#1C1C21] border border-carbon/20 dark:border-white/10 rounded-sm pl-9 pr-4 py-2 text-xs text-carbon dark:text-white focus:outline-none focus:border-carbon dark:border-white/20"
          />
        </div>
      </div>
          ...
          {filteredStudents.length === 0 ? (
            ...
            <>
              ...
              <Link 
                ...
                class="bg-gray-50 dark:bg-[#1C1C21] border border-carbon/10 dark:border-white/5 rounded-lg overflow-hidden hover:border-carbon dark:border-white/20 transition-all hover:transform hover:-translate-y-1 group"
              >
                <div class="h-32 bg-white dark:bg-[#0A0B0E] relative overflow-hidden flex justify-center items-center">
                  ...
                  {student.foto ? (
                    ...
                    <User size={60} class="text-gray-600 dark:text-gray-400 opacity-30" />
                  )}
                ...
                <div class="p-4">
                  <p class="font-body font-bold text-carbon dark:text-white uppercase text-sm truncate">{student.nombres} {student.apellidos}</p>
                  
                  <div class="mt-4 flex items-center justify-between text-[11px] border-t border-carbon/10 dark:border-white/5 pt-3">
                    <div>
                      <span class="block text-gray-500 uppercase tracking-wider text-[9px]">Grado</span>
                      <span class="text-carbon dark:text-white font-bold">{student.grado.split(' / ')[0]}</span>
                    </div>
                    <ChevronRight size={16} class="text-gray-500 group-hover:text-carbon dark:text-white transition-colors" />
                  </div>
                </div>
              </Link>
            ))}
